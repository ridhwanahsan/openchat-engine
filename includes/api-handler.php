<?php
add_action('wp_ajax_send_openchat_engine_message', 'handle_openchat_engine_message');
add_action('wp_ajax_nopriv_send_openchat_engine_message', 'handle_openchat_engine_message');

function handle_openchat_engine_message()
{
    ...
    if ($recaptcha_enabled) {
        ...
        $secret_key = trim(get_option('openchat_engine_recaptcha_secret_key'));
        if (empty($secret_key)) {
    ...
    $api_key = trim(get_option('openchat_engine_api_key'));
    $model   = trim(get_option('openchat_engine_model'));
    if (empty($model)) {
    ...
    $system_prompt = get_option('openchat_engine_system_prompt');
    ...
    if (is_wp_error($response)) {
        error_log('[OpenChat Engine] WP_Error: ' . $response->get_error_message());
    ...
    if ($http_code !== 200) {
        error_log('[OpenChat Engine] HTTP ' . $http_code . ': ' . $body);
    }
    ...
    if (isset($data['choices'][0]['message']['content'])) {
        ...
        error_log('[OpenChat Engine Analytics Debug] Session ID: ' . $session_id);
        error_log('[OpenChat Engine Analytics Debug] User Message: ' . $message);
        error_log('[OpenChat Engine Analytics Debug] Bot Response: ' . $bot_reply);
        ...
        if ($insert_result === false) {
            error_log('[OpenChat Engine Analytics Debug] Database Insert Error: ' . $wpdb->last_error);
        } else {
            error_log('[OpenChat Engine Analytics Debug] Database Insert Success. Rows affected: ' . $insert_result);
        }
...
function save_openchat_engine_email_support_request() {
    ...
    if ($insert_result === false) {
        error_log('[OpenChat Engine Email Support Debug] Database Insert Error: ' . $wpdb->last_error);
        wp_send_json_error(['message' => 'Failed to save email support request.'], 500);
    } else {
        // Send email notification to admin
        $recipient_email = get_option('openchat_engine_support_email_recipient', get_option('admin_email'));
        $subject = 'New OpenChat Engine Email Support Request';
        $body = "A new email support request has been submitted:\n\n" .
                "Client Email: " . $email . "\n" .
                "Problem Description: " . $problem;

        // --- START: Added Debugging Logs for wp_mail --- 
        error_log('[OpenChat Engine Email Support Debug] Attempting to send admin email.');
        error_log('[OpenChat Engine Email Support Debug] Admin Email: ' . $recipient_email);
        error_log('[OpenChat Engine Email Support Debug] Subject: ' . $subject);
        error_log('[OpenChat Engine Email Support Debug] Body: ' . $body);
...
function clear_openchat_engine_analytics_data() {
    global $wpdb;
    $table_name = $wpdb->prefix . 'openchat_engine_analytics';

    error_log('[OpenChat Engine Clear Analytics Debug] clear_openchat_engine_analytics_data function called.');
    ...
    try {
        ...
        if ($result === false) {
            error_log('[OpenChat Engine Clear Analytics Debug] Database TRUNCATE failed: ' . $wpdb->last_error);
            wp_send_json_error(['message' => 'Failed to clear analytics data: ' . $wpdb->last_error]);
        } else {
            error_log('[OpenChat Engine Clear Analytics Debug] Database TRUNCATE successful. Rows affected: ' . $result);
            wp_send_json_success(['message' => 'OpenChat Engine analytics data cleared successfully.']);
        }
    } catch (Exception $e) {
        error_log('[OpenChat Engine Clear Analytics Debug] Exception caught: ' . $e->getMessage());
...
function clear_openchat_engine_email_support_data() {
    ...
    try {
        ...
        if ($result === false) {
            error_log('[OpenChat Engine Clear Email Support Debug] Database TRUNCATE failed: ' . $wpdb->last_error);
            wp_send_json_error(['message' => 'Failed to clear email support data: ' . $wpdb->last_error]);
        } else {
            error_log('[OpenChat Engine Clear Email Support Debug] Database TRUNCATE successful. Rows affected: ' . $result);
            wp_send_json_success(['message' => 'OpenChat Engine email support data cleared successfully.']);
        }
    } catch (Exception $e) {
        error_log('[OpenChat Engine Clear Email Support Debug] Exception caught: ' . $e->getMessage());
        wp_send_json_error(['message' => 'An unexpected error occurred: ' . $e->getMessage()]);
    }
}
